...
int main() try
{
    ...
    while (running)
    {
        ...
        float constant = (time - last_animation_change) / animation_change_duration;
        float interpolation = (last_animation_change != 0 && constant < 1) ? constant : 1;
        ...
        for (int i = 0; i < animation.bones.size(); i++) {
            ...
            // auto translation = glm::translate(glm::mat4(1), bone.translation(t1));
            // auto scaling = glm::scale(glm::mat4(1), bone.scale(t1));
            // auto rotation = glm::toMat4(bone.rotation(t1));
            last_animation_translation[i] = glm::lerp(last_animation_translation[i], bone.translation(t1), interpolation);
            last_animation_scaling[i] = glm::lerp(last_animation_scaling[i], bone.scale(t1), interpolation);
            last_animation_rotation[i] = glm::slerp(last_animation_rotation[i], bone.rotation(t1), interpolation);

            auto translation = glm::translate(glm::mat4(1), last_animation_translation[i]);
        ...
        glUniformMatrix4x3fv(bones_location, 100, GL_FALSE, reinterpret_cast<float *>(&bones[0][0][0]));

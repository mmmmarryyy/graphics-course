...
int main() try
{
    ...
    std::vector<GLuint> objects_ids_query;
    std::vector<bool> objects_freedom;
    ...
    while (running)
    {
        ...
        if (button_down[SDLK_EQUALS])
            lod_const = std::min(50, lod_const + 1);
        ...
        GLuint first_free_query_object_index = -1;

        for (int i = 0; i < objects_ids_query.size(); i++) {
            if (objects_freedom[i]) {
                first_free_query_object_index = i;
                break;
            }
        }

        if (first_free_query_object_index == -1) {
            int query_size = objects_ids_query.size();
            objects_ids_query.push_back(query_size);
            objects_freedom.push_back(true);
            glGenQueries(1, &objects_ids_query[query_size]);
            first_free_query_object_index = query_size;
        }

        objects_freedom[first_free_query_object_index] = false;
        glBeginQuery(GL_TIME_ELAPSED, objects_ids_query[first_free_query_object_index]);
        ...
        for (int i = 0; i < 32; i++) {
            for (int j = 0; j < 32; j++) {
                glm::vec3 offset = {1.f * (i - 16), 0.f, 1.f * (j - 16)};
                aabb box = aabb(input_model.meshes[0].min + offset, input_model.meshes[0].max + offset);
                int lod = std::max(0, std::min(5, (int)std::round(glm::distance(offset, camera_position) / lod_const)));
        ...
        for (int i = 0; i < objects_ids_query.size(); i++) {
            ...
            GLint query_object_data;
            glGetQueryObjectiv(i, GL_QUERY_RESULT_AVAILABLE, &query_object_data);

            if (query_object_data == GL_FALSE) {
                continue;
            }

            glGetQueryObjectiv(i, GL_QUERY_RESULT, &query_object_data);
            std::cout << "query " << objects_ids_query[i] << " : " << query_object_data / 1e6f << " ms passed\n";
            std::cout << "draw " << instances[0].size() + instances[1].size() + instances[2].size() + instances[3].size() + instances[4].size() + instances[5].size() << " objects" << std::endl;

            objects_freedom[i] = true;
        }
        std::cout << std::flush;
    }

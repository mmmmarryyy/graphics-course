...
void main()
{
    ...
    vec3 albedo = vec3(texture(sampler, vec2(texcoord.x + cos(time), texcoord.y - sin(time))));
...
int main() try
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 1, GL_RGBA8, size/2, size/2, 0, GL_RGBA, GL_UNSIGNED_BYTE, pixels1.data());

    std::vector<std::uint32_t> pixels2(size / 4 * size / 4, 0xFF00FF00u);
    glTexImage2D(GL_TEXTURE_2D, 2, GL_RGBA8, size/4, size/4, 0, GL_RGBA, GL_UNSIGNED_BYTE, pixels2.data());

    std::vector<std::uint32_t> pixels3(size / 8 * size / 8, 0xFFFF0000u);
    glTexImage2D(GL_TEXTURE_2D, 3, GL_RGBA8, size/8, size/8, 0, GL_RGBA, GL_UNSIGNED_BYTE, pixels3.data());

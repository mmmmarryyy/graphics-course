...
void main()
{
    ...
    // float bias_const = 0.005;
    // if (in_shadow_texture)
    //     shadow_factor = (texture(shadow_map, shadow_pos.xy).r + bias_const < shadow_pos.z) ? 0.0 : 1.0;
...
bounding_box get_bounding_box(const std::vector<obj_data::vertex> &scene) {
    ...
    for(int i = 0; i < 2; i++) {
        for(int j = 0; j < 2; j++) {
            for(int k = 0; k < 2; k++) {
                res[1 * i + 2 * j + 4 * k] = glm::vec3(x_bounds[i], y_bounds[j], z_bounds[k]);
            }
        }
    }
    return res;
...
int main() try
{
    ...
    glm::vec3 center = std::accumulate(bounding_box.begin(), bounding_box.end(), glm::vec3(0.f)) / 8.f;
    ...
    while (running)
    {
        ...
        for(auto vertice : bounding_box) {
            glm::vec3 v = vertice - center;

...
uniform int need_to_apply_environment_map;
...
vec3 new_fog_addition(vec3 color) {
    float inner_camera_distance = length(camera_position - position);
    vec3 inner_camera_direction = normalize(camera_position - position);

    const int number_of_iterations = 8;
    vec3 fog_color = vec3(221, 226, 227) / 255;

    float dt = inner_camera_distance / number_of_iterations;
    float inner_optical_depth = 0.0;

    float fog_absorption = 0.0015;
    vec3 result_fog_color = vec3(0.0);

    for (int i = 0; i < number_of_iterations; i++) {
        inner_optical_depth += dt * fog_absorption;
        result_fog_color += fog_color * exp(-inner_optical_depth) * dt / inner_camera_distance;
        vec3 inner_p = camera_position + inner_camera_direction * dt * i;

        vec4 inner_ndc_p = transform * vec4(inner_p, 1.0);
        if (-1 <= inner_ndc_p.x && inner_ndc_p.x <= 1 && -1 <= inner_ndc_p.y && inner_ndc_p.y <= 1 && -1 <= inner_ndc_p.z && inner_ndc_p.z <= 1) {
            vec2 shadow_texcoord_p = inner_ndc_p.xy * 0.5 + 0.5;
            float shadow_depth_p = inner_ndc_p.z * 0.5 + 0.5;
            result_fog_color += texture(shadow_map, vec3(shadow_texcoord_p, shadow_depth_p)) * exp(-inner_optical_depth) * dt / inner_camera_distance * 3;
        }
    }

    float fog_factor = 1 - exp(-inner_optical_depth);
    return mix(color, result_fog_color, fog_factor);
}
...
void main()
{
    ...
    for (int x = -N; x <= N; ++x) {
        for (int y = -N; y <= N; ++y) {
            ...
            sum += c * texture(shadow_map, vec3(shadow_pos.xy + vec2(x,y) / vec2(textureSize(shadow_map, 0)), shadow_pos.z));
            sum_w += c;
        }
    }
    ...
    if (need_to_apply_environment_map == 1) {
        ...
        // vec4 environment_color = texture(environment_map, reflected_ray_direction); //TODO

        // vec3 view_vector = normalize(camera_position - position);
        // vec3 reflected_vector = reflect(view_vector, normal);
        // vec3 refracted_vector = refract(view_vector, normal, 1.0/1.33);
        ...
        // vec4 environment_color = mix(reflected_color, refracted_color, 0.0);
        // out_color = mix(out_color, environment_color, 1.0);
    }
...
glm::vec3 calculateSphereCenter(const std::vector<vertex>& vertices)
{
...
int main() try
{
    ...
    GLuint environment_map_location = glGetUniformLocation(program, "environment_map");
    GLuint need_to_apply_environment_map_location = glGetUniformLocation(program, "need_to_apply_environment_map");
    ...
    glm::vec3 center_of_shpere = glm::vec3(0);
    {
        auto [vertices, indices] = generate_sphere(80.f, 16);

        center_of_shpere = calculateSphereCenter(vertices);
        center_of_shpere[1] -= 80;
        std::cout << center_of_shpere.x << " " << center_of_shpere.y << " " << center_of_shpere.z << std::endl;
    ...
    int environment_map_resolution = 2048;
    ...
    for (int i = 0; i < 6; i++) {
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGBA8, environment_map_resolution, environment_map_resolution, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
    }
    ...
    GLuint environment_fbo;
    glGenFramebuffers(1, &environment_fbo);
    glBindFramebuffer(GL_FRAMEBUFFER, environment_fbo);
    glFramebufferTexture(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, environment_map, 0);
    glDrawBuffer(GL_COLOR_ATTACHMENT0);

    GLuint environment_rbo;
    glGenRenderbuffers(1, &environment_rbo);
    glBindRenderbuffer(GL_RENDERBUFFER, environment_rbo);
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT, environment_map_resolution, environment_map_resolution);
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, environment_rbo);

    std::cout << "before incomplete framebuffer" << std::endl;
    ...
    // const int point_shadow_texture_unit = 2;
    ...
    while (running)
    {
        ...
        for (int i = 0; i < 6; i++) {
            glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, environment_map, 0);

            //TODO: add code of switchToFace(i)
            if (i == 0) { //right
                x_rotate = 0;
                y_rotate = -90;
                z_rotate = 0;
            ...
            // add code of renderLowQualityScene(scene, camera):
            // add code of prepare():
            glClearColor(1.f, 1.f, 1.f, 1.f);
            glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
            // entityRenderer.render(getimportantEntities, camera, getLightDirection, NO_CLIP):
            // TODO: add code of prepare(camera, lightDir, clipPlane);
            // TODO: refactor
            ...
            // glm::mat4 inner_transform = glm::inverse(glm::mat4({
            //     {dx * light_x.x, dx * light_x.y, dx * light_x.z, 0.f},
            //     {dy * light_y.x, dy * light_y.y, dy * light_y.z, 0.f},
            //     {dz * light_z.x, dz * light_z.y, dz * light_z.z, 0.f},
            //     {c.x, c.y, c.z, 1.f}
            ...
            // glm::mat4 inner_model(1.f);

            float near = 0.1f;
            float far = 100000.f;

            glm::mat4 view(1.f);
            view = glm::rotate(view, glm::radians(camera_rotation.x), {1, 0, 0});
            view = glm::rotate(view, glm::radians(camera_rotation.y), {0, 1, 0});
            view = glm::rotate(view, glm::radians(camera_rotation.z), {0, 0, 1});
            view = glm::rotate(view, glm::radians(x_rotate), {1, 0, 0});
            view = glm::rotate(view, glm::radians(y_rotate), {0, 1, 0});
            view = glm::rotate(view, glm::radians(z_rotate), {0, 0, 1});
            view = glm::translate(view, center_of_shpere);
            ...
            glUniform1i(need_to_apply_environment_map_location, 0);
        ...
        glUniform1i(need_to_apply_environment_map_location, 1);
        glBindVertexArray(sphere_vao);
        glDrawElements(GL_TRIANGLES, sphere_index_count, GL_UNSIGNED_INT, nullptr);

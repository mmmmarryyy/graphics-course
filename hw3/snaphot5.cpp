...
vec3 new_fog_addition(vec3 color) {
    ...
    for (int i = 0; i < number_of_iterations; i++) {
        ...
        vec3 inner_p = position + inner_camera_direction * dt * i;

        vec4 inner_ndc_p = transform * vec4(inner_p, 1.0);
        inner_ndc_p /= inner_ndc_p.w;
        inner_ndc_p = inner_ndc_p * 0.5 + vec4(0.5);
        if (-1 <= inner_ndc_p.x && inner_ndc_p.x <= 1 && -1 <= inner_ndc_p.y && inner_ndc_p.y <= 1 && -1 <= inner_ndc_p.z && inner_ndc_p.z <= 1) {
            ...
            result_fog_color += texture(shadow_map, inner_ndc_p.xy).xyz * exp(-inner_optical_depth) * dt / inner_camera_distance * 3;
        }
    }
    ...
    //     // vec4 inner_ndc_p = transform * vec4(inner_p, 1.0);
    //     vec4 inner_ndc_p = transform * vec4(position + inner_camera_direction * dt * i, 1.0);
    //     inner_ndc_p /= inner_ndc_p.w;
    //     inner_ndc_p = inner_ndc_p * 0.5 + vec4(0.5);
    //     if (-1 <= inner_ndc_p.x && inner_ndc_p.x <= 1 && -1 <= inner_ndc_p.y && inner_ndc_p.y <= 1 && -1 <= inner_ndc_p.z && inner_ndc_p.z <= 1) {
    //         result_fog_color += texture(shadow_map, inner_ndc_p.xyz) * exp(-inner_optical_depth) * dt / inner_camera_distance * 3;
    //     }
    // }
...
void main()
{
    ...
    for (int x = -N; x <= N; ++x) {
        for (int y = -N; y <= N; ++y) {
            ...
            sum += c * texture(shadow_map, shadow_pos.xy + vec2(x,y) / vec2(textureSize(shadow_map, 0)));//texture(shadow_map, vec3(shadow_pos.xy + vec2(x,y) / vec2(textureSize(shadow_map, 0)), shadow_pos.z));
            sum_w += c;
        }
    }
...
int main() try
{
    ...
    while (running)
    {
        ...
        glUniform3f(ambient_location, 0.1f, 0.2f, 0.3f);

...
int main() try {
    ...
    while (running) {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            glm::mat4 view(1.f);
            view = glm::translate(view, {3.f * float(i % 3 - 1), 3.f * float(i / 3 - 1), -camera_distance});
            ...
            glUniform3f(sun_direction_location, 1.f, 0.f, 1.f);
            glUniform3f(sun_color_location, 1.0f, 0.9f, 0.8f);
            glUniform3f(point_light_position_location, cos(time), sin(time), 1.f);
            glUniform3f(point_light_color_location, 0.f, 1.f, 1.f);
            glUniform3f(point_light_attenuation_location, 1.f, 0.f, 0.01f);
            glUniform1f(glossiness_location, 4.f * float(i) / 9);
            glUniform1f(roughness_location, 0.3f * float(i) / 9);
            // std::cout << "(i / 3) = " << (i / 3) << "; (i % 3)" << (i % 3) << std::endl;
